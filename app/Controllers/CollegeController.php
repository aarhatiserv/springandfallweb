    ...
    public function apply()
 {
    ...
    public function getCollegeDetails( $country, $id ) {
        ...
        $dataCourse = $cmodel->findAll();
            ...
            // $data.push($data2[$i]);
            $db = \Config\Database::connect();
            $query = $db->query("SELECT DISTINCT  department.id as d_id, department.name as d_name, colleges.id as college_id, colleges.names as college_name, colleges.country as country, colleges.image as image, colleges.image_background as image_background, colleges.about as about, level.id as l_id, level.name as l_name FROM course INNER JOIN department ON course.department_id = department.id INNER JOIN colleges ON course.college_id = colleges.id INNER JOIN level ON course.level_id = level.id WHERE course.college_id = '".$dataCollege[0]['id']."'");
