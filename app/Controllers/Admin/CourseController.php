...
class CourseController extends Controller
{
    ...
    public function editCourse($collegeId, $id)
    {
		...
		if(!empty($session->get('username'))){
            ...
            $db = \Config\Database::connect();
            $query = $db->query("SELECT course.id as course_id, department.id as d_id, level.id as l_id,  department.name as d_name, level.name as l_name, name FROM course INNER JOIN department ON course.department_id = department.id INNER JOIN level ON course.level_id = level.id WHERE course.id = '$id'");
